...
(function (global, factory) {
  ...
  if (hasConsole) {
    ...
    welcome = function () {
      ...
      if (hasGroup) {
        console.groupCollapsed.apply(console, welcomeIntro);
      }
      console.log(message);
      if (hasGroup) {
    ...
    LinkModel__proto__.notifiedUpstream = function notifiedUpstream(
      ...
      if (startPath && this.rootLink) {
        ...
        if (this.root !== root) {
          ...
          parent.links.forEach(function (l) {
            return l.notifiedUpstream(path$1, parent.root);
          });
          parent.deps.forEach(function (d) {
            return d.handleChange(path$1);
          });
    ...
    while (i--) {
      ...
      if (item.w) {
        ...
        if (typeof prev === "string") {
          items[i - 1] = prev.replace(trailingIndent, "$1");
        }
        if (item.f) {
          if (typeof item.f[0] === "string") {
            item.f[0] = item.f[0].replace(leadingLine, "");
          }
          var last = item.f.length - 1;
          if (typeof item.f[last] === "string") {
            item.f[last] = item.f[last].replace(
              trailingIndent,
              function (_m, str) {
                hitLast = str;
                return str || "";
        ...
        var next = items[i + 1];
        if (hitLast && typeof next === "string") {
          items[i + 1] = next.replace(leadingLine, "");
        }
      }
  ...
  var StandardParser = Parser.extend({
    init: function init(str, options) {
      ...
      this.preserveStandaloneSections =
        this.preserveWhitespace && options.preserveStandaloneSections;
  ...
  function throwNoParse(method, error, instructions) {
    if (!method) {
      fatal(
        "Missing Ractive.parse - cannot parse " + error + ". " + instructions
      );
    }
  }
    ...
    if (base[0] === "@") {
      ...
      if (base === "@this" || base === "@") {
        ...
        while (
          f &&
          (!f.context ||
            (f.isRoot && f.ractive.component && (root || !f.ractive.isolated)))
        ) {
    ...
    if (isObject(keypath)) {
      var loop = function (k) {
        if (hasOwn(keypath, k)) {
          sets.push.apply(
            sets,
            gather(ractive, k, null, isolated).map(function (m) {
      ...
      sets.push.apply(
        sets,
        gather(ractive, keypath, null, isolated).map(function (m) {
      ...
      throw new Error(
        "ractive.animate(...) no longer supports objects. Instead of ractive.animate({\n  " +
          ...
          keys$$1
            .map(function (key) {
              return (
                "ractive.animate('" + key + "', " + keypath[key] + ", {...});"
              );
            })
            .join("\n") +
          "\n"
  ...
  function findAnchorsIn(item, name, result, instance) {
    if (item.isAnchor) {
      ...
      item.items.forEach(function (i) {
        return findAnchorsIn(i, name, result, instance);
      });
      ...
      item.fragment &&
      (!item.component || item.fragment.ractive === instance)
    ) {
      ...
      anchors.forEach(function (a) {
        return a.ractive === instance && result.push(a);
      });
    }
  }
  ...
  function attachChild(child, options) {
    ...
    rm.joinKey("parent", { lastLink: false }).link(
      this.viewmodel.getRactiveModel()
    );
    rm.joinKey("root", { lastLink: false }).link(
      this.root.viewmodel.getRactiveModel()
    );
  ...
  Context__proto__.findAllComponents = function findAllComponents(
    selector,
    opts
  ) {
    var result = [];
  ...
  if (!svg) {
    ...
    createElement = function (type, ns, is) {
      ...
      return is
        ? doc.createElementNS(ns, type, wrap(is))
  ...
  function getStyle(id) {
    return (
      doc &&
      (doc.querySelector('[data-ractive-css="' + id + '"]') || makeStyle(id))
    );
  }
  ...
  var RactiveModel = (function (SharedModel) {
    ...
    var RactiveModel__proto__ = (RactiveModel.prototype = Object.create(
      SharedModel && SharedModel.prototype
    ));
  ...
  var RootModel = (function (Model) {
    ...
    RootModel__proto__.getRactiveModel = function getRactiveModel() {
      return (
        this.ractiveModel ||
        (this.ractiveModel = new RactiveModel(this.ractive))
      );
    };
  ...
  function construct(ractive, options) {
    ...
    while (i--) {
      ...
      ractive[name$1] = assign(
        create(ractive.constructor.prototype[name$1]),
        options[name$1]
      );
    }
  ...
  function handler(ev) {
    ...
    if (el.listeners && (listeners = el.listeners[ev.type])) {
      var len = listeners.length;
      for (var i = 0; i < len; i++) {
        listeners[i] && listeners[i].call(this$1, ev);
      }
    }
  }
  ...
  CustomEvent__proto__.unrender = function unrender() {
    ...
    if (this.handler) {
      ...
      runloop.scheduleTask(function () {
        return this$1.handler && this$1.handler.teardown();
      });
    }
  };
  ...
  function initialise(ractive, userOptions, options) {
    ...
    if (fragment) {
      ...
      var el =
        (ractive.el =
        ractive.target =
          getElement(ractive.el || ractive.target));
  ...
  function render$1(ractive, target, anchor, occupants) {
    ...
    var promise = runloop.start();
    runloop.scheduleTask(function () {
      return hooks.render.fire(ractive);
    }, true);
    ...
    if (target) {
      (
        target.__ractive_instances__ || (target.__ractive_instances__ = [])
      ).push(ractive);
  ...
  if (win && !win.Ractive) {
    ...
    if (script) {
      opts$1 = script.getAttribute("data-ractive-options") || "";
    }
